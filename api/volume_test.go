...
func (s *S) TestVolumeList(c *check.C) {
	...
	s.mockService.VolumeService.OnListByFilter = func(ctx context.Context, f *volumeTypes.Filter) ([]volumeTypes.VolumeWithBinds, error) {
		return []volumeTypes.VolumeWithBinds{{
...
func (s *S) TestVolumeListPermissions(c *check.C) {
	...
	s.mockService.VolumeService.OnListByFilter = func(ctx context.Context, f *volumeTypes.Filter) ([]volumeTypes.VolumeWithBinds, error) {
		return []volumeTypes.VolumeWithBinds{{Volume: v2}}, nil
	}
	url := "/1.4/volumes"
	request, err := http.NewRequest("GET", url, nil)
	c.Assert(err, check.IsNil)
	...
	s.mockService.VolumeService.OnListByFilter = func(ctx context.Context, f *volumeTypes.Filter) ([]volumeTypes.VolumeWithBinds, error) {
		return []volumeTypes.VolumeWithBinds{{Volume: v1}}, nil
	}
	request, err = http.NewRequest("GET", url, nil)
	c.Assert(err, check.IsNil)
...
func (s *S) TestVolumeListBinded(c *check.C) {
	...
	s.testServer.ServeHTTP(recorder, request)
	c.Assert(recorder.Code, check.Equals, http.StatusOK)
	var result []volumeTypes.VolumeWithBinds
	err = json.Unmarshal(recorder.Body.Bytes(), &result)
	c.Assert(err, check.IsNil)
	c.Assert(result, check.DeepEquals, []volumeTypes.VolumeWithBinds{
		{
			Volume: volumeTypes.Volume{
				Name:      "v1",
				Pool:      s.Pool,
...
func (s *S) TestVolumeCreate(c *check.C) {
	...
	c.Assert(err, check.IsNil)
	c.Assert(volumes, check.DeepEquals, []volumeTypes.VolumeWithBinds{{
		Volume: volumeTypes.Volume{
			Name:      "v1",
			Pool:      s.Pool,
			TeamOwner: s.team.Name,
...
func (s *S) TestVolumeUpdate(c *check.C) {
	config.Set("volume-plans:nfs:fake:plugin", "nfs")
	defer config.Unset("volume-plans")
	s.mockService.VolumeService.OnGet = func(ctx context.Context, name string) (*volumeTypes.Volume, error) {
		return &volumeTypes.Volume{Name: name, Pool: s.Pool, TeamOwner: s.team.Name, Plan: volumeTypes.VolumePlan{Name: "nfs"}, Opts: map[string]string{"a": "b"}}, nil
	}
...
func (s *S) TestVolumeDelete(c *check.C) {
	...
	defer config.Unset("volume-plans")
	s.mockService.VolumeService.OnGet = func(ctx context.Context, name string) (*volumeTypes.Volume, error) {
		return &volumeTypes.Volume{Name: name, Pool: s.Pool, TeamOwner: s.team.Name, Plan: volumeTypes.VolumePlan{Name: "nfs"}}, nil
	}
	url := "/1.4/volumes/v1"
