...
func (s *S) TestGetEnvAllEnvs(c *check.C) {
	...
	err := s.mockService.AppEnvVar.Set(context.TODO(), &a, []appTypes.EnvVar{
...
func (s *S) TestSetEnvPublicEnvironmentVariableInTheApp(c *check.C) {
	...
	err := app.CreateApp(context.TODO(), &a, s.user)
	c.Assert(err, check.IsNil)
	var appEnvVarSetCalled bool
	s.mockService.AppEnvVar.OnSet = func(a appTypes.App, envs []appTypes.EnvVar, args appTypes.SetEnvArgs) error {
		appEnvVarSetCalled = true
		c.Assert(a.GetName(), check.Equals, "black-dog")
		c.Assert(envs, check.DeepEquals, []appTypes.EnvVar{
			{Name: "DATABASE_HOST", Value: "localhost", Public: true},
		})
		c.Assert(args.ShouldRestart, check.Equals, true)
		c.Assert(args.PruneUnused, check.Equals, false)
		c.Assert(args.ManagedBy, check.Equals, "")
		c.Assert(args.Writer, check.NotNil)
		return nil
	}
	url := fmt.Sprintf("/apps/%s/env", a.Name)
	d := apiTypes.Envs{
	...
	c.Assert(recorder.Code, check.Equals, http.StatusOK)
	c.Assert(recorder.Header().Get("Content-Type"), check.Equals, "application/x-json-stream")
	c.Assert(appEnvVarSetCalled, check.Equals, true)
	c.Assert(eventtest.EventDesc{
		Target: appTarget(a.Name),
		Owner:  s.token.GetUserName(),
...
func (s *S) TestSetEnvPublicAndPrivate(c *check.C) {
	...
	s.mockService.AppEnvVar.OnSet = func(a appTypes.App, envs []appTypes.EnvVar, args appTypes.SetEnvArgs) error {
		...
		c.Assert(a.GetName(), check.Equals, "black-dog")
		c.Assert(envs, check.DeepEquals, []appTypes.EnvVar{
			{Name: "DATABASE_HOST", Value: "localhost", Public: false, ManagedBy: "terraform"},
			{Name: "MY_DB_HOST", Value: "otherhost", Public: true, ManagedBy: "terraform"},
		})
		c.Assert(args.ShouldRestart, check.Equals, true)
		c.Assert(args.PruneUnused, check.Equals, false)
		c.Assert(args.ManagedBy, check.Equals, "terraform")
		c.Assert(args.Writer, check.NotNil)
		return nil
	}
	d := apiTypes.Envs{
...
func (s *S) TestSetEnvCanPruneOldVariables(c *check.C) {
	a := app.App{Name: "black-dog", TeamOwner: s.team.Name}
	err := app.CreateApp(context.TODO(), &a, s.user)
	c.Assert(err, check.IsNil)
	var appEnvVarSetCalled bool
	s.mockService.AppEnvVar.OnSet = func(a appTypes.App, envs []appTypes.EnvVar, args appTypes.SetEnvArgs) error {
		...
		c.Assert(args.ShouldRestart, check.Equals, true)
		c.Assert(args.PruneUnused, check.Equals, true)
		c.Assert(args.ManagedBy, check.Equals, "terraform")
		c.Assert(args.Writer, check.NotNil)
		return nil
	}
...
func (s *S) TestBindHandler(c *check.C) {
	...
	c.Assert(parts[1], check.Matches, `{"Message":"The following environment variables are available for use in your app:\\n\\n","Timestamp":".*"}`)
	c.Assert(parts[2], check.Matches, `{"Message":"- DATABASE_(USER|PASSWORD)\\n","Timestamp":".*"}`)
	c.Assert(parts[3], check.Matches, `{"Message":"- DATABASE_(USER|PASSWORD)\\n","Timestamp":".*"}`)
	c.Assert(parts[4], check.Matches, `{"Message":"- TSURU_SERVICES\\n","Timestamp":".*"}`)
	c.Assert(parts[5], check.Equals, "")
	c.Assert(recorder.Header().Get("Content-Type"), check.Equals, "application/x-json-stream")
	c.Assert(eventtest.EventDesc{
		Target: appTarget(a.Name),
		Owner:  s.token.GetUserName(),
...
func (s *S) TestUnbindHandler(c *check.C) {
	...
	err = s.conn.ServiceInstances().Insert(instance)
	c.Assert(err, check.IsNil)
	var unsetAllCalled bool
	s.mockService.AppServiceEnvVar.OnUnsetAll = func(a appTypes.App, args appTypes.UnsetAllArgs) error {
		unsetAllCalled = true
		c.Assert(a.GetName(), check.Equals, "painkiller")
		c.Assert(args.Service, check.Equals, "mysql")
		c.Assert(args.Instance, check.Equals, "my-mysql")
		c.Assert(args.ShouldRestart, check.Equals, true)
		c.Assert(args.Writer, check.NotNil)
		return nil
	}
	...
	c.Assert(unsetAllCalled, check.Equals, true)
	err = s.conn.ServiceInstances().Find(bson.M{"name": instance.Name}).One(&instance)
	c.Assert(err, check.IsNil)
...
func (s *S) TestUnbindNoRestartFlag(c *check.C) {
	...
	err := service.Create(srvc)
	c.Assert(err, check.IsNil)
	a := app.App{Name: "painkiller", TeamOwner: s.team.Name}
	err = app.CreateApp(context.TODO(), &a, s.user)
	c.Assert(err, check.IsNil)
	err = s.provisioner.AddUnits(context.TODO(), &a, 1, "web", nil, nil)
	c.Assert(err, check.IsNil)
	...
	s.mockService.AppServiceEnvVar.OnUnsetAll = func(a appTypes.App, args appTypes.UnsetAllArgs) error {
		...
		c.Assert(args.Service, check.Equals, "mysql")
		c.Assert(args.Instance, check.Equals, "my-mysql")
		c.Assert(args.ShouldRestart, check.Equals, false)
		c.Assert(args.Writer, check.NotNil)
		return nil
	}
...
func (s *S) TestRegisterUnit(c *check.C) {
	...
	err := s.mockService.AppEnvVar.Set(context.TODO(), &a, []appTypes.EnvVar{
		{Name: "MY_VAR_1", Value: "value1", Public: true},
	}, appTypes.SetEnvArgs{})
	c.Assert(err, check.IsNil)
	err = app.CreateApp(context.TODO(), &a, s.user)
	c.Assert(err, check.IsNil)
