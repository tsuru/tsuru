...
func (s *ServiceSuite) SetUpSuite(c *C) {
	...
	_, err := Db.Exec("CREATE TABLE 'services' ('id' INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, 'service_type_id' integer,'name' varchar(255))")
	c.Check(err, IsNil)

	_, err = Db.Exec("CREATE TABLE 'service_types' ('id' INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, 'name' varchar(255), 'charm' varchar(255))")
	c.Check(err, IsNil)

	_, err = Db.Exec("CREATE TABLE 'service_apps' ('id' INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, 'service_id' integer, 'app_id' integer)")
	c.Check(err, IsNil)
...
func (s *ServiceSuite) TearDownTest(c *C) {
	...
	Db.Exec("DELETE FROM service_apps")
...
func (s *ServiceSuite) TestCreateHandler(c *C) {
	...
	rows, err := Db.Query("SELECT id, service_type_id, name FROM services WHERE name = 'some_service'")
	c.Check(err, IsNil)
...
func (s *ServiceSuite) TestDeleteHandler(c *C) {
	...
	rows, err := Db.Query("SELECT count(*) FROM services WHERE name = 'Mysql'")
	c.Check(err, IsNil)
...
func (s *ServiceSuite) TestBindHandler(c *C) {
	...
	rows, err := Db.Query("SELECT count(*) FROM service_apps WHERE service_id = ? AND app_id = ?", se.Id, a.Id)
	c.Check(err, IsNil)
