...
func getStateFromOpts(driverOptions *types.DriverOptions) (state, error) {
	...
	d.NodePoolID = options.GetValueFromDriverOptions(driverOptions, types.StringType, "node-pool", "nodePool").(string)
	...
	d.EnableNetworkPolicyConfig, _ = options.GetValueFromDriverOptions(driverOptions, types.BoolPointerType, "enable-network-policy-config", "enableNetworkPolicyConfig").(*bool)
	...
	d.NodeConfig.ImageType = options.GetValueFromDriverOptions(driverOptions, types.StringType, "image-type", "imageType").(string)
