...
func (c *subnetCleanup) Get(a AzureClient) (err error) {
	c.ref, err = a.subnetsClient().Get(context.TODO(), c.rg, c.vnet, c.name, "")
	return err
}
...
func (c *vnetCleanup) Get(a AzureClient) (err error) {
	c.ref, err = a.virtualNetworksClient().Get(context.TODO(), c.rg, c.name, "")
	return err
}
...
func (c *avSetCleanup) Get(a AzureClient) (err error) {
	c.ref, err = a.availabilitySetsClient().Get(context.TODO(), c.rg, c.name)
	return err
}

func (c *avSetCleanup) Delete(a AzureClient) error {
	_, err := a.availabilitySetsClient().Delete(context.TODO(), c.rg, c.name)
	return err
}
