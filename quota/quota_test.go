...
func (Suite) TestReserve(c *gocheck.C) {
	...
	n, err := Reserve("last@dreamtheater.com", "dt/1")
	c.Assert(err, gocheck.IsNil)
	c.Assert(n, gocheck.Equals, 1)
	n, err = Reserve("last@dreamtheater.com", "dt/0")
	c.Assert(err, gocheck.IsNil)
	c.Assert(n, gocheck.Equals, 1)
	n, err = Reserve("last@dreamtheater.com", "dt/0")
	conn, err := db.Conn()
...
func (Suite) TestReserveRepeatedItems(c *gocheck.C) {
	...
	c.Assert(err, gocheck.IsNil)
	defer Delete("spirit@dreamtheater.com")
	n, err := Reserve("spirit@dreamtheater.com", "spirit/0")
	c.Assert(err, gocheck.IsNil)
	c.Assert(n, gocheck.Equals, 1)
	n, err = Reserve("spirit@dreamtheater.com", "spirit/0")
	c.Assert(err, gocheck.IsNil)
...
func (Suite) TestReserveMultiple(c *gocheck.C) {
	...
	n, err := Reserve("barbarian@elp.com", "b/0", "b/1", "b/2")
	c.Assert(err, gocheck.IsNil)
...
func (Suite) TestReserveMultipleExceed(c *gocheck.C) {
	...
	n, err := Reserve("pebble@elp.com", "p/0", "p/1", "p/2")
...
func (Suite) TestReserveQuotaExceeded(c *gocheck.C) {
	...
	n, err := Reserve("change@dreamtheater.com", "change/0")
	c.Assert(err, gocheck.IsNil)
	c.Assert(n, gocheck.Equals, 1)
	n, err = Reserve("change@dreamtheater.com", "change/1")
	c.Assert(err, gocheck.Equals, ErrQuotaExceeded)
	c.Assert(n, gocheck.Equals, 0)
}

func (Suite) TestReserveQuotaNotFound(c *gocheck.C) {
	n, err := Reserve("home@dreamtheater.com", "something")
	c.Assert(err, gocheck.Equals, ErrQuotaNotFound)
	c.Assert(n, gocheck.Equals, 0)
}
...
func (Suite) TestRelease(c *gocheck.C) {
	...
	_, err = Reserve("beyond@yes.com", "beyond/0")
	c.Assert(err, gocheck.IsNil)
	err = Release("beyond@yes.com", "beyond/0")
	c.Assert(err, gocheck.IsNil)
	_, err = Reserve("beyond@yes.com", "beyond/1")
	c.Assert(err, gocheck.IsNil)
}
...
func (Suite) TestSetQuota(c *gocheck.C) {
	...
	_, err = Reserve("survival@yes.com", "survival/0")
	c.Assert(err, gocheck.IsNil)
	err = Set("survival@yes.com", 10)
	c.Assert(err, gocheck.IsNil)
	_, err = Reserve("survival@yes.com", "survival/1")
	c.Assert(err, gocheck.IsNil)
}
...
func (Suite) TestItemsGreaterThanLimit(c *gocheck.C) {
	...
	_, err = Reserve("coming@yes.com", "coming/0")
	c.Assert(err, gocheck.IsNil)
	_, err = Reserve("coming@yes.com", "coming/1")
	c.Assert(err, gocheck.IsNil)
	_, err = Reserve("coming@yes.com", "coming/2")
	c.Assert(err, gocheck.IsNil)
	err = Set("coming@yes.com", 2)
	c.Assert(err, gocheck.IsNil)
	_, err = Reserve("coming@yes.com", "coming/3")
	c.Assert(err, gocheck.Equals, ErrQuotaExceeded)
}
...
func (Suite) TestItems(c *gocheck.C) {
	...
	_, err = Reserve("sorry@evergrey.com", "sorry/0")
	c.Assert(err, gocheck.IsNil)
	_, err = Reserve("sorry@evergrey.com", "sorry/1")
	c.Assert(err, gocheck.IsNil)
	items, err = Items("sorry@evergrey.com")
	c.Assert(err, gocheck.IsNil)
	c.Assert(items, gocheck.DeepEquals, []string{"sorry/0", "sorry/1"})
	_, err = Reserve("sorry@evergrey.com", "sorry/2")
	c.Assert(err, gocheck.IsNil)
	_, err = Reserve("sorry@evergrey.com", "sorry/3")
	c.Assert(err, gocheck.IsNil)
	items, err = Items("sorry@evergrey.com")
	c.Assert(err, gocheck.IsNil)
