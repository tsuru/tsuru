...
func (Suite) TestManagerKeys(c *check.C) {
	...
	c.Check(err, check.Equals, repository.ErrUserNotFound)
	err = manager.UpdateKey("gopher", repository.Key{Name: "name", Body: "changed body"})
	c.Check(err, check.IsNil)
	keys, err = manager.ListKeys("gopher")
	c.Check(err, check.IsNil)
	c.Check(keys, check.DeepEquals, []repository.Key{{Name: "name", Body: "changed body"}})
	err = manager.UpdateKey("someuser", repository.Key{Body: "bbb"})
	c.Assert(err, check.Equals, repository.ErrUserNotFound)
	err = manager.UpdateKey("gopher", repository.Key{Name: "nameee", Body: "changed body"})
