...
func (s *TeamSuite) TestInsertTeam(c *check.C) {
	t := types.Team{Name: "teamname", CreatingUser: "me@example.com"}
...
func (s *TeamSuite) TestFindAllTeams(c *check.C) {
	err := s.TeamStorage.Insert(types.Team{Name: "corrino"})
	c.Assert(err, check.IsNil)
	err = s.TeamStorage.Insert(types.Team{Name: "fenring"})
	c.Assert(err, check.IsNil)
...
func (s *TeamSuite) TestDeleteTeamNotFound(c *check.C) {
	err := s.TeamStorage.Delete(types.Team{Name: "myteam"})
	c.Assert(err, check.Equals, types.ErrTeamNotFound)
}
