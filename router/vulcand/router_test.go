...
func (s *S) TestRemoveBackend(c *check.C) {
	...
	err = vRouter.AddBackend(context.TODO(), app)
	c.Assert(err, check.IsNil)
	backends, err := s.engine.GetBackends()
	c.Assert(err, check.IsNil)
	...
	err = vRouter.RemoveBackend(context.TODO(), app)
	c.Assert(err, check.IsNil)
	backends, err = s.engine.GetBackends()
	c.Assert(err, check.IsNil)
	c.Assert(backends, check.HasLen, 0)
...
func (s *S) TestRemoveBackendNotExist(c *check.C) {
	...
	c.Assert(err, check.IsNil)
	c.Assert(backends, check.HasLen, 0)
	err = vRouter.RemoveBackend(context.TODO(), routertest.FakeApp{Name: "myapp"})
...
func (s *S) TestAddRoutes(c *check.C) {
	...
	err = vRouter.AddRoutes(context.TODO(), app, []*url.URL{u1})
	c.Assert(err, check.IsNil)
	err = vRouter.AddRoutes(context.TODO(), app, []*url.URL{u2})
	c.Assert(err, check.IsNil)
...
func (s *S) TestRemoveRoute(c *check.C) {
	...
	c.Assert(err, check.IsNil)
	c.Assert(servers, check.HasLen, 2)
	err = vRouter.RemoveRoutes(context.TODO(), app, []*url.URL{u1})
	c.Assert(err, check.IsNil)
	servers, err = s.engine.GetServers(engine.BackendKey{Id: "tsuru_myapp"})
	c.Assert(err, check.IsNil)
...
func (s *S) TestSetCName(c *check.C) {
	...
	c.Assert(ok, check.Equals, true)
	err = cnameRouter.SetCName(context.TODO(), "myapp.cname.example.com", app)
	c.Assert(err, check.IsNil)
...
func (s *S) TestUnsetCName(c *check.C) {
	...
	cnameRouter.UnsetCName(context.TODO(), "myapp.cname.example.com", app)
	frontends, err = s.engine.GetFrontends()
	c.Assert(err, check.IsNil)
	c.Assert(frontends, check.HasLen, 1)
...
func (s *S) TestUnsetCNameNotExist(c *check.C) {
	...
	cnameRouter, ok := vRouter.(router.CNameRouter)
	c.Assert(ok, check.Equals, true)
	err = cnameRouter.UnsetCName(context.TODO(), "myapp.cname.example.com", app)
...
func (s *S) TestAddr(c *check.C) {
	...
	err = vRouter.AddBackend(context.TODO(), app)
	c.Assert(err, check.IsNil)
	addr, err := vRouter.Addr(context.TODO(), app)
	c.Assert(err, check.IsNil)
...
func (s *S) TestAddrNotExist(c *check.C) {
	...
	c.Assert(err, check.IsNil)
	c.Assert(backends, check.HasLen, 0)
	addr, err := vRouter.Addr(context.TODO(), routertest.FakeApp{Name: "myapp"})
	c.Assert(err, check.Equals, router.ErrBackendNotFound)
...
func (s *S) TestSwap(c *check.C) {
	vRouter, err := router.Get(context.TODO(), "vulcand")
	c.Assert(err, check.IsNil)
	app1 := routertest.FakeApp{Name: "myapp1"}
	app2 := routertest.FakeApp{Name: "myapp2"}
	err = vRouter.AddBackend(context.TODO(), app1)
	c.Assert(err, check.IsNil)
	u1, _ := url.Parse("http://1.1.1.1:111")
	u2, _ := url.Parse("http://2.2.2.2:222")
	err = vRouter.AddRoutes(context.TODO(), app1, []*url.URL{u1})
	c.Assert(err, check.IsNil)
	err = vRouter.AddBackend(context.TODO(), app2)
	c.Assert(err, check.IsNil)
	err = vRouter.AddRoutes(context.TODO(), app2, []*url.URL{u2})
	c.Assert(err, check.IsNil)
	err = vRouter.Swap(context.TODO(), app1, app2, false)
	c.Assert(err, check.IsNil)
...
func (s *S) TestRoutes(c *check.C) {
	...
	err = vRouter.AddRoutes(context.TODO(), app, []*url.URL{u2})
	c.Assert(err, check.IsNil)
	routes, err := vRouter.Routes(context.TODO(), app)
	c.Assert(err, check.IsNil)
...
func (s *S) TestHealthCheckFailure(c *check.C) {
	...
	err := tsurutest.WaitCondition(time.Second, func() bool {
		_, err := tsuruNet.Dial15Full60ClientNoKeepAlive.Get(s.vulcandServer.URL)
		return err != nil
	})
	c.Assert(err, check.IsNil)
	got, err := router.Get(context.TODO(), "vulcand")
