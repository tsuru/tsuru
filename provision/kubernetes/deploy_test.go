...
func (s *S) TestServiceManagerDeployServiceProcessStartupcheckTimeoutExceeded(c *check.C) {
	config.Set("docker:healthcheck:max-time", 1)
	defer config.Unset("docker:healthcheck:max-time")
	config.Set("kubernetes:deployment-progress-timeout", 10)
	defer config.Unset("kubernetes:deployment-progress-timeout")
	...
	c.Assert(buf.String(), check.Matches, `(?s).*---- Updating units \[p1\] \[version 1\] ----.* Initial Check Timeout of 8s exceeded .*ROLLING BACK AFTER FAILURE.*`)
}
