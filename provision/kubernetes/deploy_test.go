...
func (s *S) TestServiceManagerDeployService(c *check.C) {
	...
	c.Assert(err, check.IsNil)
	dep, err := s.client.Clientset.AppsV1beta2().Deployments(s.client.Namespace(a.GetName())).Get("myapp-p1", metav1.GetOptions{})
	c.Assert(err, check.IsNil)
	one := int32(1)
	ten := int32(10)
	...
	srv, err := s.client.CoreV1().Services(s.client.Namespace(a.GetName())).Get("myapp-p1", metav1.GetOptions{})
	c.Assert(err, check.IsNil)
	...
	srvHeadless, err := s.client.CoreV1().Services(s.client.Namespace(a.GetName())).Get("myapp-p1-units", metav1.GetOptions{})
	c.Assert(err, check.IsNil)
...
func (s *S) TestServiceManagerDeployServiceWithHC(c *check.C) {
	...
	c.Assert(err, check.IsNil)
	dep, err := s.client.Clientset.AppsV1beta2().Deployments(s.client.Namespace(a.GetName())).Get("myapp-web", metav1.GetOptions{})
	c.Assert(err, check.IsNil)
	...
	dep, err = s.client.Clientset.AppsV1beta2().Deployments(s.client.Namespace(a.GetName())).Get("myapp-p2", metav1.GetOptions{})
	c.Assert(err, check.IsNil)
...
func (s *S) TestServiceManagerDeployServiceWithRegistryAuth(c *check.C) {
	...
	secrets, err := s.client.CoreV1().Secrets(s.client.Namespace(a.GetName())).List(metav1.ListOptions{})
	c.Assert(err, check.IsNil)
...
func (s *S) TestServiceManagerDeployServiceProgressMessages(c *check.C) {
	...
	s.client.PrependReactor("create", "deployments", func(action ktesting.Action) (bool, runtime.Object, error) {
		...
		go func() {
			...
			s.client.AppsV1beta2().Deployments(s.clusterClient.Namespace(a.GetName())).Update(&depCopy)
		}()
		return false, nil, nil
	})
	...
	c.Assert(err, check.IsNil)
	_, err = s.client.Clientset.AppsV1beta2().Deployments(s.client.Namespace(a.GetName())).Get("myapp-web", metav1.GetOptions{})
	c.Assert(err, check.IsNil)
...
func (s *S) TestServiceManagerDeployServiceWithNodeContainers(c *check.C) {
	...
	daemon, err := s.client.Clientset.AppsV1beta2().DaemonSets(s.client.Namespace(a.GetName())).Get("node-container-bs-all", metav1.GetOptions{})
	c.Assert(err, check.IsNil)
...
func (s *S) TestCreateBuildPodContainers(c *check.C) {
	...
	c.Assert(err, check.IsNil)
	pods, err := s.client.Core().Pods(s.client.Namespace(a.GetName())).List(metav1.ListOptions{})
	c.Assert(err, check.IsNil)
...
func (s *S) TestCreateDeployPodProgress(c *check.C) {
	...
	s.client.PrependReactor("create", "pods", func(action ktesting.Action) (bool, runtime.Object, error) {
		...
		go func() {
			...
			s.clusterClient.CoreV1().Pods(s.clusterClient.Namespace(a.GetName())).Update(&podCopy)
		}()
		return false, nil, nil
	})
	s.client.PrependWatchReactor("events", func(action ktesting.Action) (handled bool, ret watch.Interface, err error) {
...
func (s *S) TestServiceManagerDeployServiceRollbackFullTimeout(c *check.C) {
	...
	_, err = s.client.CoreV1().Events(s.client.Namespace(a.GetName())).Create(&apiv1.Event{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "pod.evt1",
			Namespace: s.client.Namespace(a.GetName()),
		},
...
func (s *S) TestServiceManagerDeployServiceRollbackHealthcheckTimeout(c *check.C) {
	...
	s.client.PrependReactor("create", "deployments", func(action ktesting.Action) (bool, runtime.Object, error) {
		...
		go func() {
			_, repErr := s.client.Clientset.AppsV1beta2().ReplicaSets(s.client.Namespace(a.GetName())).Create(&v1beta2.ReplicaSet{
				ObjectMeta: metav1.ObjectMeta{
...
func (s *S) TestServiceManagerRemoveService(c *check.C) {
	...
	_, err = s.client.Clientset.AppsV1beta2().ReplicaSets(s.client.Namespace(a.GetName())).Create(&v1beta2.ReplicaSet{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "myapp-p1-xxx",
			Namespace: s.client.Namespace(a.GetName()),
	...
	c.Assert(err, check.IsNil)
	_, err = s.client.CoreV1().Pods(s.client.Namespace(a.GetName())).Create(&apiv1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "myapp-p1-xyz",
			Namespace: s.client.Namespace(a.GetName()),
			Labels:    expectedLabels,
	...
	c.Assert(err, check.IsNil)
	deps, err := s.client.Clientset.AppsV1beta2().Deployments(s.client.Namespace(a.GetName())).List(metav1.ListOptions{})
	c.Assert(err, check.IsNil)
	c.Assert(deps.Items, check.HasLen, 0)
	srvs, err := s.client.CoreV1().Services(s.client.Namespace(a.GetName())).List(metav1.ListOptions{})
	c.Assert(err, check.IsNil)
	c.Assert(srvs.Items, check.HasLen, 0)
	pods, err := s.client.CoreV1().Pods(s.client.Namespace(a.GetName())).List(metav1.ListOptions{})
	c.Assert(err, check.IsNil)
	c.Assert(pods.Items, check.HasLen, 0)
	replicas, err := s.client.Clientset.AppsV1beta2().ReplicaSets(s.client.Namespace(a.GetName())).List(metav1.ListOptions{})
	c.Assert(err, check.IsNil)
