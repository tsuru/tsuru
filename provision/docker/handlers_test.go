...
func (s *HandlersSuite) TestRemoveNodeHandler(c *gocheck.C) {
	...
	c.Assert(err, gocheck.IsNil)
	_, err = dCluster.Register("host.com:4243", nil)
	c.Assert(err, gocheck.IsNil)
...
func (s *HandlersSuite) TestRemoveNodeHandlerWithoutRemoveIaaS(c *gocheck.C) {
	...
	dCluster, err = cluster.New(nil, &cluster.MapStorage{})
	c.Assert(err, gocheck.IsNil)
	_, err = dCluster.Register(fmt.Sprintf("http://%s:4243", machine.Address), nil)
	c.Assert(err, gocheck.IsNil)
...
func (s *HandlersSuite) TestListNodeHandler(c *gocheck.C) {
	...
	dCluster, err = cluster.New(nil, &cluster.MapStorage{})
	c.Assert(err, gocheck.IsNil)
	_, err = dCluster.Register("host1.com:4243", map[string]string{"pool": "pool1"})
	c.Assert(err, gocheck.IsNil)
	_, err = dCluster.Register("host2.com:4243", map[string]string{"pool": "pool2", "foo": "bar"})
	c.Assert(err, gocheck.IsNil)
