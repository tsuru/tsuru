...
func (s *S) TestContainerCreate(c *check.C) {
	...
	app.Memory = 15
	app.SetEnv(bindTypes.EnvVar{Name: "A", Value: "myenva"})
	app.SetEnv(bindTypes.EnvVar{Name: "ABCD", Value: "other env"})
	...
	container, err := dcli.InspectContainerWithOptions(docker.InspectContainerOptions{ID: cont.ID})
	c.Assert(err, check.IsNil)
	...
	c.Assert(container.Config.CPUShares, check.Equals, int64(1024))
	...
	c.Assert(container.HostConfig.CPUShares, check.Equals, int64(1024))
...
func (s *S) TestContainerCreateCustomLog(c *check.C) {
	...
	for i, testData := range testCases {
		...
		c.Assert(err, check.IsNil)
		dockerContainer, err := client.InspectContainerWithOptions(docker.InspectContainerOptions{ID: cont.ID})
		c.Assert(err, check.IsNil)
...
func (s *S) TestContainerCreateForDeploy(c *check.C) {
	...
	c.Assert(dockerContainer.HostConfig.CPUShares, check.Equals, int64(1024))
...
func (s *S) TestContainerRemove(c *check.C) {
	...
	_, err = client.InspectContainerWithOptions(docker.InspectContainerOptions{ID: container.ID})
...
func (s *S) TestContainerStop(c *check.C) {
	...
	c.Assert(err, check.IsNil)
	dockerContainer, err := s.cli.InspectContainerWithOptions(docker.InspectContainerOptions{ID: cont.ID})
	c.Assert(err, check.IsNil)
