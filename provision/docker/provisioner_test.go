...
func (s *S) TestDeployFailureSecondStepWithUnit(c *gocheck.C) {
	var (
		p   dockerProvisioner
		buf bytes.Buffer
	)
	app := testing.NewFakeApp("app", "python", 0)
	app.AddUnit(&testing.FakeUnit{Name: "app/0"})
	app.AddUnit(&testing.FakeUnit{Name: "c-0955"})
	...
	c.Assert(fexec.ExecutedCmd("docker", []string{"rm", "c-0955"}), gocheck.Equals, false)
	c.Assert(app.ProvisionUnits(), gocheck.HasLen, 1)
}

func (s *S) TestDeployFailureSecondStepWithoutUnit(c *gocheck.C) {
	var (
		p   dockerProvisioner
		buf bytes.Buffer
	)
