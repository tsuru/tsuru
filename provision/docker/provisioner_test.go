...
func (s *S) TestCollectStatus(c *gocheck.C) {
	...
	rtesting.FakeRouter.AddRoute("ashamed", fmt.Sprintf("http://127.0.0.1:%s", listenPort))
...
`
	...
	c.Assert(fexec.ExecutedCmd("ssh-keygen", []string{"-R", "127.0.0.1"}), gocheck.Equals, true)
