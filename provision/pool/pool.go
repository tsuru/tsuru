...
func AddPool(opts AddPoolOptions) error {
	...
	if opts.Public || opts.Default {
		return SetPoolConstraint(&PoolConstraint{PoolExpr: opts.Name, Type: ConstraintTypeTeam, Values: []string{"*"}})
	}
	return nil
}
...
func PoolUpdate(name string, opts UpdatePoolOptions) error {
	...
	if (opts.Public != nil && *opts.Public) || (opts.Default != nil && *opts.Default) {
		errConstraint := SetPoolConstraint(&PoolConstraint{PoolExpr: name, Type: ConstraintTypeTeam, Values: []string{"*"}})
