...
func generateDirtyOutput(ip string) string {
	var template = `2012-12-14 17:19:28,235 INFO Connecting to environment...
2012-12-14 17:19:29,455 INFO Connected to environment.
machines:
  0:
    agent-state: running
  ...
  the_infanta:
    ...
    units:
      the_infanta/1:
        ...
        public-address: server-1086.novalocal
2012-12-14 17:19:29,665 INFO 'status' command finished successfully`
	return fmt.Sprintf(template, ip, ip, ip, ip, ip, ip)
}

func generateOutput(ip string) string {
	var template = `machines:
  0:
    agent-state: running
    dns-name: %s
    instance-id: i-00000376
