...
func (s *S) TestListFilterMany(c *check.C) {
	...
	checkFilters(&event.Filter{KindType: event.KindTypePermission, KindName: []string{"kind"}}, nil)
	checkFilters(&event.Filter{KindType: event.KindTypeInternal, KindName: []string{"healer"}, Sort: "_id"}, allEvts[3:len(allEvts)-1])
